		...
		B_Connect.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e){
				...
				try{
					...
					signal = true;
					TA_Chat.append("Pokusavam da se konektujem na remote IP:"
							+ ip + " i port:" + p + "\n");
		...
		TF_Poruka.addKeyListener(new KeyListener(){
			public void keyPressed(KeyEvent e){
				...
				if(tipka == KeyEvent.VK_ENTER){
					...
					TA_Chat.setCaretPosition(TA_Chat.getText().length());
					posaljiPoruku(TF_Nick.getText() + ">" + poruka2); // salji
																		// poruku
					TF_Poruka.setText("");
				}
			}
		...
		public void run(){
			try{
				try{
					...
					poruka = (String) ulaz.readObject(); // prima poruku od
															// severa po prvi
															// put
			...
			try{
				do{
					try{
						...
						/*
						 * This part (if - else if) of the code was done a
						 * little lame, but it works as it should when someone
						 * read message log in our chat, and scrollbar is for
						 * ex. in middle of text area, we don't want our
						 * scrollbar jump to bottom of text area, when we
						 * receive new message +16px appears sometimes I do not
						 * know why yet, but it does't hurt so our scrollbar
						 * will be auto-scrolling to bottom, when current
						 * location of scrollbar is more then maximum size or
						 * equal to size of our text area
						 */
