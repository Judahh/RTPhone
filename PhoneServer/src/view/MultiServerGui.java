...
public class MultiServerGui extends JFrame{
	...
	static ServerSocket			serverSocket;												// we
																							// need
																							// this
																							// line
																							// to
	...
	static Socket				socket;													// and
																							...
																							// remote
																							// connections
	static int					clientNiz[]	= new int[100];								// array
																							// "clientNiz"
																							// is
																							// unique
																							// id
	...
	// array to do job
	static ObjectInputStream	ulaz[]		= new ObjectInputStream[clientNiz.length + 1];	// set
																							// input
																							// stream
																							// to
																							// 100
																							...
																							// up
																							// everything)
	static ObjectOutputStream	izlaz[]		= new ObjectOutputStream[clientNiz.length + 1]; // set
																							// output
																							// stream
																							// to
																							// 100
	...
	void postaviKomponente(){
		// this is old method, that i using to put components on container, this
		// can be replaced with setLayout, like Grid or FlowLayout
	...
	void postaviSlusaoca(){
		B_StartServer.addActionListener(new ActionListener(){
	...
	// method where server received message from one client and send it to all
	// other clients
	void posaljiPoruku(int cli, String por){
		for(int i = 0; i < clientNiz.length; i++) // for loop trying to send
													// message from server to
													// all clients
		{
			if(clientNiz[i] != 0) // this line stop server to send messages to
									// offline clients (if "clientNiz[X] = 0"
									// don't try to send him message, because
									// that slot is empty)
			{
				if(cli != i + 1) // don't repeat messages (when for ex. client_1
									// send message to all clients, this line
									// stop server to send same message back to
									// client_1)
				{
					try{
		...
		public void run(){
			while(true){
				try{
					...
					TA_Chat.append("Konekcija primljena od: "
							+ socket.getInetAddress().getHostName() + "\n");
					TA_Chat.setCaretPosition(TA_Chat.getText().length());

					int broj = 0; // we need broj to catch client id
					for(int i = 0; i < clientNiz.length; i++){
						if(clientNiz[i] == 0) // find first 0 or "empty slot" in
												// array and set client_id
												// number
						{
							clientNiz[i] = i + 1;
							broj = i;
					...
					// here we set output and input stream for every client
					// individually
					izlaz[clientNiz[broj]] = new ObjectOutputStream(
							socket.getOutputStream());
					izlaz[clientNiz[broj]].flush();
					ulaz[clientNiz[broj]] = new ObjectInputStream(
							socket.getInputStream());

					TA_Chat.append("Client:[" + clientNiz[broj]
							+ "] Uspesno konektovan!\n"); // print message to
															// all users that
															// client[id] is
															// connected on
					...
					posaljiJednom(clientNiz[broj],
							"Uspesno si se konektovao na server!"); // send
																	// message
																	// to
																	// client[id]
																	// that is
																	// connected
																	// on server
					nicks[clientNiz[broj]] = (String) ulaz[clientNiz[broj]]
							.readObject(); // recive nick from client
					posaljiServer(nicks[clientNiz[broj]]
							+ " je online sada !!!"); // obavesti sve da se novi
														// klijent konektovao na
														// server

					ReceiveMsgs ob2 = new ReceiveMsgs(clientNiz[broj],
							"StartReceiveMsgs_" + clientNiz[broj]); // make new
																	// thread
																	// for every
																	// new
																	// client
	...
	public class ReceiveMsgs implements Runnable{
		...
		public void run(){
			do{
				try{
					poruka = (String) ulaz[C_id].readObject(); // primi poruku -
																// try to
																// receive
																// messages
					TA_Chat.append("<Join_" + C_id + ">" + poruka + "\n"); // print
																			...
																			// on
																			// server
					if(SP_Scroll.getVerticalScrollBar().getValue() >= (SP_Scroll
							.getVerticalScrollBar().getMaximum() - SP_Scroll
							.getVerticalScrollBar().getVisibleAmount()))
						TA_Chat.setCaretPosition(TA_Chat.getText().length());
					else if((SP_Scroll.getVerticalScrollBar().getValue() + 16) >= (SP_Scroll
							.getVerticalScrollBar().getMaximum() - SP_Scroll
							.getVerticalScrollBar().getVisibleAmount()))
						TA_Chat.setCaretPosition(TA_Chat.getText().length());
				}catch(Exception e){
					poruka = "cao";
				}// server can't receive message from client, set poruka to
					// "cao", exit from do-while loop and then close thread with
					// client

				if(poruka.equals("cao")) // if poruka = "cao" , close thread and
											// connection with offline client
				{
					...
					if(SP_Scroll.getVerticalScrollBar().getValue() >= (SP_Scroll
						...
						TA_Chat.setCaretPosition(TA_Chat.getText().length());
							...
							.getVerticalScrollBar().getVisibleAmount()))
						TA_Chat.setCaretPosition(TA_Chat.getText().length());
					posaljiPoruku(C_id, "<" + nicks[C_id] + "> is now offline!"); // send
																					// to
																					// all
																					// that
																					// client
					...
					clientNiz[C_id - 1] = 0; // if someone leave or disconnect,
												// clear that slot and set it to
												// 0 in array, so other clients
												// can join
					nicks[C_id] = ""; // free nick name slot
					try{
						ulaz[C_id].close(); // close input connection with
											// client
						izlaz[C_id].close(); // close output connection with
												// client
					}catch(Exception ex){
					}
				}
